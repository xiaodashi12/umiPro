                ...
                id="table">
                    <el-table-column label="操作" align="center">
                        <template slot-scope="scope">
                             <el-button size="mini" type="primary" @click="chongZheng(scope.row)" v-if="scope.row.status==1 && scope.row.needEquilibrium==0">冲正</el-button>
                        </template>
                    </el-table-column>
...
export default {
    ...
    methods: {
        chongZheng(row){
            this.$confirm('此操作将冲正这条数据, 是否继续?', '提示', {
                confirmButtonText: '确定',
                cancelButtonText: '取消',
                type: 'warning'
            }).then(() => {
                let params = {
                    url: api['equilibrium'].url,
                    method: 'post',
                    data: {
                        orderId:row.orderId,
                        cardId:row.cpuCardId,
       ...
       addNewUser(){
        ...
        showDataList(){
            let params = {
                url: api['findOrderDetailBranchToday'].url,
                method: 'post',
                data: {
